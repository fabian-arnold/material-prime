MaterialPrime.widget.BaseTree = PrimeFaces.widget.BaseWidget.extend({

    init: function(cfg) {
        this._super(cfg);
        this.cfg.highlight = (this.cfg.highlight !== false);
    ...
    bindContextMenu : function(menuWidget, targetWidget, targetId, cfg) {
        var nodeContentSelector = targetId + ' .tree-selectable',
                    ...
                    PrimeFaces.ajax.Response.handle(responseXML, status, xhr, {
                        ...
                        handle: function(content) {
                            ...
                            if(this.cfg.draggable) {
                                this.makeDraggable(nodeChildrenContainer.find('span.tree-header'));
                            }
    ...
    nodeClick: function(event, nodeContent) {
        ...
        if($(event.target).is(':not(.material-tree-toggler)')) {
            var node = nodeContent.parent(),
                selectable = nodeContent.hasClass('tree-selectable');
    ...
    partialCheck: function(checkbox) {
        var box = checkbox.children('.ui-chkbox-box'),
            ...
            treeNode = checkbox.closest('.tree-item'),
            rowKey = this.getRowKey(treeNode);

        treeNode.find('> .tree-header > .tree-item-label').removeClass('ui-state-highlight');
 ...
 */
MaterialPrime.widget.VerticalTree = MaterialPrime.widget.BaseTree.extend({

    init: function(cfg) {
        this._super(cfg);

        this.container = this.jq.children('.tree');
        this.cfg.rtl = this.jq.hasClass('rtl');
        this.cfg.collapsedIcon = this.cfg.rtl ? 'keyboard_arrow_down' : 'keyboard_arrow_right';
    ...
    bindEvents: function() {
        var $this = this,
            togglerSelector = '.material-tree-toggler',
            nodeLabelSelector = '.tree-selectable .tree-item-label',
            nodeContentSelector = '.tree-header';
        ...
        this.jq.off('click.tree-toggle', togglerSelector)
            .on('click.tree-toggle', togglerSelector, null, function(e) {
                ...
                if(toggleIcon.html() ==$this.cfg.collapsedIcon)
        ...
        if(this.isCheckboxSelection()) {
            ...
            this.jq.off('mouseout.tree-checkbox mouseover.tree-checkbox click.tree-checkbox', checkboxSelector)
                .on('mouseout.tree-checkbox', checkboxSelector, null, function() {
                    $(this).removeClass('ui-state-hover').parent().siblings('span.tree-item-label').removeClass('ui-state-hover');
                })
                .on('mouseover.tree-checkbox', checkboxSelector, null, function() {
                    $(this).addClass('ui-state-hover').parent().siblings('span.tree-item-label').addClass('ui-state-hover');
                });
        }
    ...
    bindKeyEvents: function() {
        ...
        this.jq.off('keydown.tree blur.tree', '.tree-item-label').on('keydown.tree', '.tree-item-label', null, function(e) {
            if(!$this.focusedNode) {
                return;
            }
            ...
            switch(e.which) {
                ...
                case keyCode.RIGHT:
                    if(!$this.focusedNode.hasClass('tree-item-leaf')) {
                        var rowkey = $this.focusedNode.data('rowkey').toString(),
                            keyLength = rowkey.length;
                ...
                case keyCode.SPACE:
                    if($this.cfg.selectionMode) {
                        var selectable = $this.focusedNode.children('.tree-header').hasClass('tree-selectable');
            ...
            .on('blur.tree', '.tree-item-label', null, function(e) {
                if($this.focusedNode) {
    ...
    searchDown: function(node) {
        ...
        else if(node.hasClass('tree-item-leaf') && node.closest('ul').parent('li').length == 0){
            nodeToFocus = node;
        }
        else {
    ...
    collapseNode: function(node) {
        var _self = this,
            nodeContent = node.find('> .tree-header'),
            toggleIcon = nodeContent.find('> .material-tree-toggler'),
            nodeType = node.data('nodetype'),
            nodeIcon = toggleIcon.nextAll('i.tree-icons'),
            iconState = this.cfg.iconStates[nodeType],
            childrenContainer = node.children('.ui-treenode-children');

        //aria
        nodeContent.find('> .tree-item-label').attr('aria-expanded', false);

        toggleIcon.html(_self.cfg.collapsedIcon);
    ...
    //@Override
    showNodeChildren: function(node) {
        var nodeContent = node.find('> .tree-header'),
            toggleIcon = nodeContent.find('> .material-tree-toggler'),
            nodeType = node.data('nodetype'),
            nodeIcon = toggleIcon.nextAll('i.tree-icons'),
            iconState = this.cfg.iconStates[nodeType];
        ...
        //aria
        nodeContent.find('> .tree-item-label').attr('aria-expanded', true);

        toggleIcon.html("keyboard_arrow_down");
    ...
    unselectAllNodes: function() {
        this.selections = [];
        this.jq.find('.tree-item-label.ui-state-highlight').each(function() {
            $(this).removeClass('ui-state-highlight').closest('.tree-item').attr('aria-selected', false);
        });
    },

    selectNode: function(node, silent) {
        node.attr('aria-selected', true)
            .find('> .tree-header > .tree-item-label').removeClass('ui-state-hover').addClass('ui-state-highlight');
    ...
    unselectNode: function(node, silent) {
        ...
        find('> .tree-header > .tree-item-label').removeClass('ui-state-highlight ui-state-hover');
    ...
    toggleCheckboxNode: function(node) {
        var $this = this,
            checkbox = node.find('> .tree-header > .ui-chkbox'),
        ...
        if(this.cfg.propagateUp) {
            node.parents('li.tree-item-parent').each(function() {
                var parentNode = $(this),
                    parentsCheckbox = parentNode.find('> .tree-header > .ui-chkbox'),
    ...
    check: function(checkbox) {
        ...
        checkbox.siblings('span.tree-item-label').addClass('ui-state-highlight').removeClass('ui-state-hover');
    },

    uncheck: function(checkbox) {
        this._super(checkbox);
        checkbox.siblings('span.tree-item-label').removeClass('ui-state-highlight');
    },

    initDraggable: function() {
        this.makeDraggable(this.jq.find('span.tree-header'));
    },
        ...
        elements.droppable({
            ...
            drop: function(event, ui) {
                var dragSource = $(ui.draggable.data('dragsourceid')).data('widget'),
                    ...
                    dropSource = $this,
                    dropPoint = $(this),
                    dropNode = dropPoint.closest('li.tree-item-parent'),
                    ...
                    oldParentNode = targetDragNode.parent().closest('li.tree-item-parent'),
    ...
    makeDropNodes: function(elements) {
        ...
        elements.droppable({
            ...
            over: function(event, ui) {
                $(this).children('.tree-item-label').addClass('ui-state-hover');
            },
            out: function(event, ui) {
                $(this).children('.tree-item-label').removeClass('ui-state-hover');
            },
            drop: function(event, ui) {
                var dragSource = $(ui.draggable.data('dragsourceid')).data('widget'),
                    dragMode = ui.draggable.data('dragmode'),
                ...
                ui.helper.remove();
                droppable.children('.tree-item-label').removeClass('ui-state-hover');
    ...
    updateDragDropBindings: function(node) {
        ...
        var subtreeDropNodeContents = node.find('span.tree-header');
    ...
    updateRowKeys: function() {
        var children = this.jq.find('> ul.tree > li.ui-treenode');
    ...
    updateChildrenRowKeys: function(children, rowkey) {
        ...
        children.each(function(i) {
            ...
            if(childNode.hasClass('tree-item-parent')) {
    ...
    makeLeaf: function(node) {
        node.removeClass('tree-item-parent').addClass('tree-item-leaf');
        node.find('> .tree-header > .material-tree-toggler').addClass('tree-item-leaf-icon').removeClass('material-tree-toggler ui-icon ui-icon-triangle-1-s');
        node.children('.ui-treenode-children').hide().children().remove();
    },

    makeParent: function(node) {
        node.removeClass('tree-item-leaf').addClass('tree-item-parent');
        node.find('> span.tree-header > span.tree-item-leaf-icon').removeClass('tree-item-leaf-icon').addClass('material-tree-toggler ui-icon ui-icon-triangle-1-e');
    ...
    unselectSubtree: function(node) {
        ...
        if(this.isCheckboxSelection()) {
            var checkbox = node.find('> .tree-header > .ui-chkbox');
            ...
            node.find('.tree-item-label.ui-state-highlight').each(function() {
    ...
    propagateDNDCheckbox: function(node) {
        ...
        var parent = node.parent().closest('.tree-item-parent');
        if(parent.length) {
    ...
    getFirstNode: function() {
        return this.jq.find('> ul.tree > li:first-child');
    },

    getNodeLabel: function(node) {
        return node.find('> span.tree-header > span.tree-item-label');
    },
...
/**
 * MaterialPrime Horizontal Tree Widget
 */
MaterialPrime.widget.HorizontalTree = MaterialPrime.widget.BaseTree.extend({
    ...
    bindEvents: function() {
        var $this = this,
            ...
            togglerSelector = '.material-tree-toggler',
            nodeContentSelector = '.tree-header.tree-selectable';
    ...
    showNodeChildren: function(node) {
        ...
        var childrenContainer = node.next(),
            toggleIcon = node.find('> .tree-header > .material-tree-toggler'),
            nodeType = node.data('nodetype'),
            iconState = this.cfg.iconStates[nodeType];

        if(iconState) {
            toggleIcon.nextAll('i.tree-icons').removeClass(iconState.collapsedIcon).addClass(iconState.expandedIcon);
        }
    ...
    collapseNode: function(node) {
        ...
        if(iconState) {
            toggleIcon.nextAll('i.tree-icons').addClass(iconState.collapsedIcon).removeClass(iconState.expandedIcon);
        }
    ...
    selectNode: function(node, silent) {
        node.removeClass('ui-treenode-unselected').addClass('ui-treenode-selected').children('.tree-header').removeClass('ui-state-hover').addClass('ui-state-highlight');
    ...
    unselectNode: function(node, silent) {
        ...
        node.removeClass('ui-treenode-selected').addClass('ui-treenode-unselected').children('.tree-header').removeClass('ui-state-highlight');
    ...
    unselectAllNodes: function() {
        this.selections = [];
        this.jq.find('.tree-header.ui-state-highlight').each(function() {
            $(this).removeClass('ui-state-highlight').closest('.tree-item').attr('aria-selected', false);
        });
    },
        ...
        if(this.cfg.propagateUp) {
            node.parents('td.ui-treenode-children-container').each(function() {
                var childrenContainer = $(this),
                    parentNode = childrenContainer.prev('.tree-item-parent'),
                    parentsCheckbox = parentNode.find('> .tree-header > .ui-chkbox'),
    ...
    check: function(checkbox) {
        this._super(checkbox);
        checkbox.parent('.tree-header').addClass('ui-state-highlight').removeClass('ui-state-hover');
    },

    uncheck: function(checkbox) {
        this._super(checkbox);
        checkbox.parent('.tree-header').removeClass('ui-state-highlight');
    },
    ...
    partialCheck: function(checkbox) {
        ...
        treeNode.find('> .tree-header').removeClass('ui-state-highlight');
        icon.html("minus");
        treeNode.removeClass('ui-treenode-selected ui-treenode-unselected').addClass('ui-treenode-hasselected').attr('aria-checked', false).attr('aria-selected', false);
